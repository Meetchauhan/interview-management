...
function Call({ interview }: InterviewProps) {
  ...
  const startConversation = async () => {
    ...
    // Get the count of attempts for this email and interview
    const attempts = await ResponseService.getAttemptCountByEmail(
      interview.id,
      email
    );
    ...
    // Check if the user has already attempted 3 times
    if (attempts >= 3) {
      setIsOldUser(true);
      setLoading(false);
...
return;
    ...
    // Only check for respondents list, not for previous email usage
    // This allows users to retry up to 3 times
    const OldUser = interview?.respondents && !interview?.respondents.includes(email);
    ...
    if (OldUser) {
      ...
      console.log("interview_id", interview);
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      ...
      <div className="bg-white rounded-md md:w-[80%] w-[90%]">
              ...
              <div className="w-fit min-w-[400px] max-w-[400px] mx-auto mt-2  border border-indigo-200 rounded-md p-2 m-2 bg-slate-50  absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
                <div>
                  <div className="p-2 font-normal text-base mb-4 whitespace-pre-line">
                    ...
                    <p className="text-lg font-semibold text-center">
                      {attemptCount >= 3 
                        ? "You have reached the maximum number of attempts (3) for this interview."
        ...
        <a
          ...
          href="https://www.metizsoft.com/"
          target="_blank"
        >
          <div className="text-center text-md font-semibold mr-2  ">
            Powered by{" "}
            <span className="font-bold">
              Metiz<span className="text-indigo-600">IQ</span>
            </span>
          </div>
